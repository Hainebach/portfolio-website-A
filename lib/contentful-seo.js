...
export async function fetchSEOMetadata(pageIdentifier = null) {
  try {
    ...
    const matchedEntry = entries.find(entry => 
      entry.fields.pageTitle?.toLowerCase().includes(pageIdentifier.toLowerCase())
    );
...
export function generatePageMetadata(siteSettings, seoMetadata, defaultTitle = '', defaultDescription = '') {
  const siteName = siteSettings?.siteName || 'Green Graphik';
  const siteDescription = siteSettings?.siteDescription || 'Amichai Green Graphic designer';
  ...
  const description = seoMetadata?.metaDescription || defaultDescription || siteDescription;
